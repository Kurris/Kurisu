...
public class RedisCacheLockRegressionTests
{
    ...
    public async Task LockAsync_ShouldRenewLeaseOnReentry_WhenModeIsFixedExpiry()
    {
        ...
        var firstHandler = await lockable.LockAsync(lockKey, options);
    ...
    public async Task LockAsync_ShouldNotReuseStaleScope_AfterLimitedRenewalLockExpires()
    {
        ...
        var contenderHandler = await contenderLockable.LockAsync(lockKey, RedisCacheTestSupport.BuildLockOptions(TimeSpan.FromSeconds(3)));
    ...
    public async Task LockAsync_ShouldThrowArgumentNullException_WhenTimeModeHandlerMissing()
    {
        ...
        await Assert.ThrowsAsync<ArgumentNullException>(() => lockable.LockAsync($"test:lock:missing-time-handler:{Guid.NewGuid():N}", options));
    }
    ...
    public async Task LockAsync_ShouldThrowArgumentNullException_WhenRetryStrategyMissing()
    {
        ...
        try
        {
            await Assert.ThrowsAsync<ArgumentNullException>(() => lockable.LockAsync(lockKey, options));
        }
        finally
        {
            await cache.KeyDeleteAsync(lockKey);
    ...
    public async Task LockAsync_ShouldMarkLostOwnership_WhenReentryDetectsStolenLock()
    {
        ...
        var handler = await lockable.LockAsync(lockKey, options);
        ...
        var reentry = await lockable.LockAsync(lockKey, options);
    ...
    public async Task LockAsync_ShouldExpireGracefully_WhenRenewalQuotaExhausted()
    {
        ...
        var lockable = serviceProvider.GetRequiredService<ILockable>();
        var otherLockable = otherServiceProvider.GetRequiredService<ILockable>();
        ...
        // 其他实例可获取同一把锁，证明原锁已过期
        var otherHandler = await otherLockable.LockAsync(lockKey, RedisCacheTestSupport.BuildLockOptions(TimeSpan.FromSeconds(2)));
    ...
    public void RedisLock_ShouldThrow_WhenExpiryNotPositive()
    {
        Assert.Throws<ArgumentException>(() =>
        {
            ...
            _ = lockable.LockAsync($"test:lock:invalid-expiry:{Guid.NewGuid():N}", options).GetAwaiter().GetResult();
        });
    }
    ...
    public async Task LockAsync_ShouldHitNotAcquiredGuard_WhenReenteringAfterLoopDetectsStolenLock()
    {
        ...
        var reentry = await lockable.LockAsync(lockKey, new DistributedLockAcquisitionOptions
        {
            TimeModeHandler = LockTimeModeHandler.FixedExpiry(TimeSpan.FromSeconds(1)),
            RetryStrategy = new DefaultLockRetryStrategy(0)
        });
