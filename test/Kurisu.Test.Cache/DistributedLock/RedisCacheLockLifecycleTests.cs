...
public class RedisCacheLockLifecycleTests
{
    ...
    public async Task LockAsync_ShouldAutoReleaseAfterExpiry_WhenNotDisposed()
    {
        ...
        var contenderLockable = contenderServiceProvider.GetRequiredService<ILockable>();
        var lockKey = $"test:lock:auto-expire:{Guid.NewGuid():N}";
        await cache.KeyDeleteAsync(lockKey);

        var firstHandler = await lockable.LockAsync(lockKey, RedisCacheTestSupport.BuildLockOptions(TimeSpan.FromMilliseconds(800)));
        Assert.True(firstHandler.Acquired);
        Assert.True(await cache.KeyExistsAsync(lockKey));

        await Task.Delay(1400);

        var secondHandler = await contenderLockable.LockAsync(lockKey, RedisCacheTestSupport.BuildLockOptions(TimeSpan.FromSeconds(3), retryCount: 1, enableRetry: false));
    ...
    public async Task LockAsync_ShouldNotDeleteNewOwnerLock_WhenOldOwnerDisposesAfterExpiry()
    {
        ...
        var lockable = serviceProvider.GetRequiredService<ILockable>();
        var anotherLockable = anotherServiceProvider.GetRequiredService<ILockable>();
        var lockKey = $"test:lock:no-wrong-delete:{Guid.NewGuid():N}";
        await cache.KeyDeleteAsync(lockKey);

        var firstHandler = await lockable.LockAsync(lockKey, RedisCacheTestSupport.BuildLockOptions(TimeSpan.FromMilliseconds(700)));
        Assert.True(firstHandler.Acquired);

        await Task.Delay(1300);

        var secondHandler = await anotherLockable.LockAsync(lockKey, RedisCacheTestSupport.BuildLockOptions(TimeSpan.FromSeconds(3), retryCount: 1, enableRetry: false));
        Assert.True(secondHandler.Acquired);
        Assert.True(await cache.KeyExistsAsync(lockKey));
    ...
    public async Task LockAsync_ShouldKeepAlive_WhenNoExplicitExpiryAndTaskRunsLongerThanDefaultTtl()
    {
        ...
        var holder = await lockable.LockAsync(lockKey, RedisCacheTestSupport.BuildLockOptions());
        Assert.True(holder.Acquired);

        await Task.Delay(7500);
        Assert.True(await cache.KeyExistsAsync(lockKey));

        var contender = await contenderLockable.LockAsync(lockKey, RedisCacheTestSupport.BuildLockOptions(TimeSpan.FromSeconds(2), retryCount: 1, enableRetry: false));
