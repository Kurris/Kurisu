...
public class RedisCacheLockFailureTests
{
    ...
    public async Task LockAsync_ShouldBeIdempotent_WhenDisposeCalledMultipleTimes()
    {
        ...
        var handler = await lockable.LockAsync(lockKey, RedisCacheTestSupport.BuildLockOptions(TimeSpan.FromSeconds(3)));
    ...
    public async Task LockAsync_ShouldReturnFailedHandler_WhenRetryIsDisabled()
    {
        ...
        var handler = await lockable.LockAsync(lockKey, RedisCacheTestSupport.BuildLockOptions(TimeSpan.FromSeconds(10), enableRetry: false));
    ...
    public async Task LockAsync_ShouldRetryOnce_WhenRetryCountIsOne()
    {
        ...
        var handler = await lockable.LockAsync(lockKey, RedisCacheTestSupport.BuildLockOptions(TimeSpan.FromSeconds(3), retryCount: 15));
    ...
    public async Task LockAsync_ShouldNotDeleteExternalLock_WhenFailedHandlerIsDisposed()
    {
        ...
        var handler = await lockable.LockAsync(lockKey, RedisCacheTestSupport.BuildLockOptions(TimeSpan.FromSeconds(10), retryCount: 1, enableRetry: false));
    ...
    public async Task LockAsync_ShouldSucceedAfterTransientContention_WhenRetryEnabled()
    {
        ...
        var handler = await lockable.LockAsync(lockKey, RedisCacheTestSupport.BuildLockOptions(TimeSpan.FromSeconds(3), retryCount: 10));
