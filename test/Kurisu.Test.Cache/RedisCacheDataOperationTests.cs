...
public class RedisCacheDataOperationTests
{
    ...
    public async Task SetAsync_WithExpiry_ShouldExpire()
    {
        ...
        // GetAsync在key不存在时会因Newtonsoft反序列化null而抛ArgumentNullException
    ...
    public async Task GetAsync_ShouldReturnDefault_WhenKeyNotExists()
    {
        ...
        // GetAsync在key不存在时Newtonsoft反序列化null会抛ArgumentNullException
        await Assert.ThrowsAsync<ArgumentNullException>(() => cache.GetAsync<TestDto>(key));
    }
