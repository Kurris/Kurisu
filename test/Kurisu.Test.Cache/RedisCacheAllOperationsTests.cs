...
public class RedisCacheAllOperationsTests
{
    ...
    [Fact(DisplayName = "StringSet泛型同步序列化+StringGet泛型反序列化")]
    public void StringSet_StringGet_Generic_Sync()
    {
        using var sp = BuildServiceProvider();
        var cache = sp.GetRequiredService<RedisCache>();
        var key = $"s:gen:{Guid.NewGuid():N}";
        cache.StringSet(key, new ValObj { N = 2, S = "sync" });
        var r = cache.StringGet<ValObj>(key);
        Assert.NotNull(r);
        Assert.Equal(2, r.N);
        cache.KeyDelete(key);
    }
    ...
    [Fact(DisplayName = "StringSetAsync泛型+StringGetAsync泛型序列化往返")]
    public async Task StringSetAsync_StringGetAsync_Generic()
    {
        using var sp = BuildServiceProvider();
        var cache = sp.GetRequiredService<RedisCache>();
        var key = $"str:gen:{Guid.NewGuid():N}";
        await cache.StringSetAsync(key, new ValObj { N = 1, S = "a" });
        var r = await cache.StringGetAsync<ValObj>(key);
        Assert.NotNull(r);
        Assert.Equal(1, r.N);
        await cache.KeyDeleteAsync(key);
    }
