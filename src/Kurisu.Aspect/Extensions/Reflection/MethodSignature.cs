...
{
    private static readonly ConcurrentDictionary<ValueTuple<MethodBase,string>, int> signatures = new ConcurrentDictionary<ValueTuple<MethodBase, string>, int>();
    ...
    public MethodSignature(MethodBase method, string name)
    {
        ...
        Name = name;
        _signature = signatures.GetOrAdd(ValueTuple.Create(method, name), GetSignatureCode);
    }
    ...
    private static int GetSignatureCode(ValueTuple<MethodBase, string> pair)
    {
