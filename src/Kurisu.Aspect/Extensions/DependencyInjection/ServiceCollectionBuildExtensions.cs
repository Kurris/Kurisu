...
public static class ServiceCollectionBuildExtensions
{
    ...
    private static Func<IServiceProvider, object> CreateFactory(ServiceDescriptor descriptor, Type proxyType)
    {
        ...
        return provider =>
        {
            ...
            var proxyInstance = reflector.Invoke(aspectActivatorFactory, instance);
            // var propertyEnableInjectAttributeType = provider.GetRequiredService<AspectConfiguration>().PropertyEnableInjectAttributeType;
            // foreach (var propertyInfo in implementationType.GetProperties())
            // {
            //     var propertyReflector = propertyInfo.GetReflector();
            //     if (propertyReflector.IsDefined(propertyEnableInjectAttributeType))
            //     {
            //         propertyReflector.SetValue(o.GetType().GetField("_implementation", BindingFlags.NonPublic | BindingFlags.Instance)
            //             .GetReflector().GetValue(o), provider.GetService(propertyInfo.PropertyType));
            //     }
            // }
