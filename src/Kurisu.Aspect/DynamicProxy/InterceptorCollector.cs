...
public sealed class InterceptorCollector : IInterceptorCollector
{
    ...
    private readonly AspectCaching<InterceptorCollector, IEnumerable<IInterceptor>> _aspectCaching;
    ...
    public InterceptorCollector(
        ...
        AspectCaching<InterceptorCollector, IEnumerable<IInterceptor>> aspectCaching)
    {
        ...
        if (aspectCaching == null)
        {
            throw new ArgumentNullException(nameof(aspectCaching));
        }
    ...
    public IEnumerable<IInterceptor> Collect(MethodInfo serviceMethod, MethodInfo implementationMethod)
    {
        ...
        return _aspectCaching.GetOrAdd(GetKey(serviceMethod, implementationMethod),
            key => HandleInjector(CollectFromService(serviceMethod)
