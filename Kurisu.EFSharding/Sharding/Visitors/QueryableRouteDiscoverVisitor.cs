...
public class QueryableRouteShardingTableDiscoverVisitor<TKey> : ShardingExpressionVisitor
{
    ...
    private readonly BaseShardingMetadata _entityMetadata;
    private readonly Func<TKey, ShardingOperatorEnum, string, Func<string, bool>> _keyToTailWithFilter;

    private LambdaExpression _entityLambdaExpression;
    private readonly ShardingPredicateResult _noShardingPredicateResult = new(false, null);
    private bool isIgnoreQueryFilter;
    private RoutePredicateExpression _where = RoutePredicateExpression.Default;

    public QueryableRouteShardingTableDiscoverVisitor(BaseShardingMetadata entityMetadata
        , Func<TKey, ShardingOperatorEnum, string, Func<string, bool>> keyToTailWithFilter)
    {
    ...
    private bool IsShardingKey(Expression expression, out ShardingPredicateResult shardingPredicateResult)
    {
        ...
        {
            if (member.Expression?.Type == _entityMetadata.ClrType)
            {
                realMember = member;
            }
                ...
                )
        {
            if (unaryExpression.Operand is MemberExpression m &&
                m.Expression?.Type == _entityMetadata.ClrType)
            {
                realMember = m;
            }
        }
        ...
        {
            bool isShardingKey;
            if (_entityMetadata.IsTableMetadata)
            {
                isShardingKey = _entityMetadata.Properties.ContainsKey(realMember.Member.Name);
            }
            else
            {
                isShardingKey = _entityMetadata.Properties.ContainsKey(realMember.Member.Name);
        ...
        out MemberExpression realMember)
    {
        ...
        {
            if (unaryExpression.Operand.Type == _entityMetadata.ClrType)
            {
                realMember = member;
                return true;
            }
    ...
    private void CombineEntityLambdaExpression(MethodCallExpression node)
    {
        if (node.Arguments[1] is UnaryExpression unaryExpression)
        {
            ...
            {
                if (lambdaExpression.Parameters[0].Type == _entityMetadata.ClrType)
                {
                    if (_entityLambdaExpression == null)
                    {
    ...
    private RoutePredicateExpression ResolveInFunc(MethodCallExpression methodCallExpression, bool @in)
    {
        if (methodCallExpression.IsEnumerableContains())
        {
            ...
            if (shardingPredicateResult.IsShardingKey)
            {
                ...
                if (arrayObject != null)
                {
                    ...
                    if (arrayObject is IEnumerable<TKey> enumerableObj)
                    {
        ...
        else if (methodCallExpression.IsStringContains())
        {
            if (IsShardingKey(methodCallExpression.Object, out var shardingPredicateResult))
            {
                if (methodCallExpression.Arguments.Count == 1)
                {
                    ...
                    var keyToTailWithFilter = _keyToTailWithFilter((TKey) shardingValue, ShardingOperatorEnum.AllLike,
                        shardingPredicateResult.ShardingPropertyName);
        ...
        else if (methodCallExpression.IsStringStartWith())
        {
            if (IsShardingKey(methodCallExpression.Object, out var shardingPredicateResult))
            {
                ...
                {
                    var shardingValue = GetExpressionValue(methodCallExpression.Arguments[0]);
                    var keyToTailWithFilter = _keyToTailWithFilter((TKey) shardingValue, ShardingOperatorEnum.StartLike,
                        shardingPredicateResult.ShardingPropertyName);
                    return new RoutePredicateExpression(keyToTailWithFilter);
                }
            }
        ...
        else if (methodCallExpression.IsStringEndWith())
        {
            if (IsShardingKey(methodCallExpression.Object, out var shardingPredicateResult))
            {
                ...
                {
                    var shardingValue = GetExpressionValue(methodCallExpression.Arguments[0]);
                    var keyToTailWithFilter = _keyToTailWithFilter((TKey) shardingValue, ShardingOperatorEnum.EndLike,
                        shardingPredicateResult.ShardingPropertyName);
                    return new RoutePredicateExpression(keyToTailWithFilter);
                }
            }
        ...
        else if (methodCallExpression.IsNamedEquals())
        {
            ...
            if (shardingPredicateResult.IsShardingKey)
            {
                ...
                {
                    var keyToTailWithFilter = _keyToTailWithFilter((TKey) shardingValue, ShardingOperatorEnum.Equal,
                        shardingPredicateResult.ShardingPropertyName);
                    return new RoutePredicateExpression(keyToTailWithFilter);
                }
            }
        ...
        Expression conditionExpression, ExpressionType expressionType)
    {
        if (ExpressionCanGetValue(conditionExpression))
        {
            ...
            return new RoutePredicateExpression(_keyToTailWithFilter((TKey) value, op, shardingPropertyName));
        }
    ...
    private RoutePredicateExpression ParsePropertyCondition(BinaryExpression binaryExpression)
    {
        ...
        if (IsShardingKey(binaryExpression.Right, out var predicateRightResult))
        {
        ...
        var left = RoutePredicateExpression.Default;
        var right = RoutePredicateExpression.Default;
        ...
        return binaryExpression.NodeType switch
        {
            ...
            _ => RoutePredicateExpression.Default
        };
    }
}
